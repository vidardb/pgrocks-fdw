...
static void KVWorkerMain(int argc, char *argv[]) {
    ...
    do {
        ...
        switch (func) {
            case OPEN:
                OpenResponse(ptr->area + sizeof(FuncName));
                break;
            case CLOSE:
                CloseResponse(ptr->area + sizeof(FuncName));
                break;
            case COUNT:
                CountResponse(ptr->area + sizeof(FuncName));
                break;
            case GETITER:
...
uint64 CountRequest(Oid relationId, SharedMem *ptr) {
    ...
    uint64 count;
    SemWait(&ptr->worker[1], __func__);
    memcpy(&count, ptr->area + sizeof(FuncName), sizeof(uint64));

...
#ifdef VIDARDB
static int
GetAttrCount(Oid foreignTableId)
{
...
static int32
GetBatchCapacity(Oid relationId)
{
				...
				TableReadState * readState)
{
	...
	/*
	 * We can push down this qual if: - The operatory is = - The qual is on
	 * the key column
	 */
...
#ifdef VIDARDB
static void
DeserializeTupleByColumn(StringInfo key,
						 StringInfo val,
...
static List *
PlanForeignModify(PlannerInfo *plannerInfo,
				  ModifyTable *plan,
...
kv_fdw_validator(PG_FUNCTION_ARGS)
{
	...
	/*
	 * if (list_length(options_list) > 0) { ereport(ERROR,
	 * (errcode(ERRCODE_FDW_INVALID_OPTION_NAME), errmsg("invalid options"),
